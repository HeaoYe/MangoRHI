...
namespace MangoRHI {
    ...
    Result VulkanSwapchain::create() {
        ...
        vulkan_context->get_extent().width = std::clamp<u32>(details.capabilities.currentExtent.width, details.capabilities.minImageExtent.width, details.capabilities.maxImageExtent.width);
        vulkan_context->get_extent().height = std::clamp<u32>(details.capabilities.currentExtent.height, details.capabilities.minImageExtent.height, details.capabilities.maxImageExtent.height);
